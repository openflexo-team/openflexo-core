/* This file was generated by SableCC (http://www.sablecc.org/). */

package org.openflexo.foundation.fml.parser.node;

import org.openflexo.foundation.fml.parser.analysis.*;

@SuppressWarnings("nls")
public final class AGtTypeParameters extends PTypeParameters
{
    private TLt _lt_;
    private PTypeParameterList _typeParameterList_;
    private TGt _gt_;

    public AGtTypeParameters()
    {
        // Constructor
    }

    public AGtTypeParameters(
        @SuppressWarnings("hiding") TLt _lt_,
        @SuppressWarnings("hiding") PTypeParameterList _typeParameterList_,
        @SuppressWarnings("hiding") TGt _gt_)
    {
        // Constructor
        setLt(_lt_);

        setTypeParameterList(_typeParameterList_);

        setGt(_gt_);

    }

    @Override
    public Object clone()
    {
        return new AGtTypeParameters(
            cloneNode(this._lt_),
            cloneNode(this._typeParameterList_),
            cloneNode(this._gt_));
    }

    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAGtTypeParameters(this);
    }

    public TLt getLt()
    {
        return this._lt_;
    }

    public void setLt(TLt node)
    {
        if(this._lt_ != null)
        {
            this._lt_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._lt_ = node;
    }

    public PTypeParameterList getTypeParameterList()
    {
        return this._typeParameterList_;
    }

    public void setTypeParameterList(PTypeParameterList node)
    {
        if(this._typeParameterList_ != null)
        {
            this._typeParameterList_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._typeParameterList_ = node;
    }

    public TGt getGt()
    {
        return this._gt_;
    }

    public void setGt(TGt node)
    {
        if(this._gt_ != null)
        {
            this._gt_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._gt_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._lt_)
            + toString(this._typeParameterList_)
            + toString(this._gt_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._lt_ == child)
        {
            this._lt_ = null;
            return;
        }

        if(this._typeParameterList_ == child)
        {
            this._typeParameterList_ = null;
            return;
        }

        if(this._gt_ == child)
        {
            this._gt_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._lt_ == oldChild)
        {
            setLt((TLt) newChild);
            return;
        }

        if(this._typeParameterList_ == oldChild)
        {
            setTypeParameterList((PTypeParameterList) newChild);
            return;
        }

        if(this._gt_ == oldChild)
        {
            setGt((TGt) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
