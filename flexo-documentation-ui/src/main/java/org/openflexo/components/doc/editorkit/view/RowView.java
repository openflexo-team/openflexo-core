...
public class RowView extends BoxView {
    ...
    public void paint(Graphics g, Shape a) {
        ...
        int n=getViewCount();
        FlexoStyledDocument.RowElement row=(FlexoStyledDocument.RowElement)this.getElement();
    ...
    public float getPreferredSpan(int axis) {
        float span=0;
        if (axis==View.X_AXIS) {
            FlexoStyledDocument.RowElement row=(FlexoStyledDocument.RowElement)getElement();
            span=row.getWidth();
        }
        else {
            span=1;
