...
public class DeprecatedWizardDialog extends FlexoDialog implements ActionListener {

	private static final Logger logger = FlexoLogger.getLogger(DeprecatedWizardDialog.class.getPackage().getName());
	...
	public DeprecatedWizardDialog(Frame owner, FlexoWizard wizard) {
		super(owner, true);
	...
	private void updateCurrentPage() {
		...
		/*if (wizard.getCurrentStep().getUserInterface() == null) {
			wizard.getCurrentStep().initUserInterface(mainPanel);
		}
		mainPanel.add(wizard.getCurrentStep().getUserInterface());*/

		mainPanel.add(wizard.getCurrentStep().getJComponent());

		pageTitle.setText(wizard.getCurrentStep().getTitle());
		updateControls();
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == cancel) {
			...
			WizardStep nextPage = wizard.getNextPage(wizard.getCurrentStep());
			if (nextPage != null) {
				wizard.setCurrentStep(nextPage);
				updateCurrentPage();
			}
		} else if (e.getSource() == previous) {
			WizardStep previousPage = wizard.getPreviousStep(wizard.getCurrentStep());
			if (previousPage != null) {
				wizard.setCurrentStep(previousPage);
				updateCurrentPage();
			}
		}
	}
