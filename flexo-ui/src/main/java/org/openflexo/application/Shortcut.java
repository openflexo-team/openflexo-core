 ...
 *
 * Note: This class represents a single shortcut, contains the factory to obtain shortcut objects from, manages shortcuts and shortcut
 * collisions, and finally manages loading and saving shortcuts to/from the preferences.
 *
...
public final class Shortcut {
	...
	protected static final Logger logger = Logger.getLogger(Shortcut.class.getPackage().getName());
	...
	/**
	 * true if it got assigned what was requested. (Note: modifiers will be ignored in favour of group when loading it from the preferences
	 * then.)
	 */
	...
	/**
	 * true if the user requested this shortcut to be set to its default value (will happen on next restart, as this shortcut will not be
	 * saved to the preferences)
	 */
	 ...
	 * 
	 * @param assignedUser
	 *            {@code true} if the user has changed this shortcut
	 */
	...
	private void saveDefault() {
		// TODO one day
		/*Config.getPref().getList("shortcut.entry." + shortText,
				Arrays.asList(longText, String.valueOf(requestedKey), String.valueOf(requestedGroup), String.valueOf(requestedKey),
	...
	private boolean save() {
		...
		/*if (isAutomatic() || isReset() || !isAssignedUser()) {
			...
			return Config.getPref().putList("shortcut.entry." + shortText,
					Arrays.asList(longText, String.valueOf(requestedKey), String.valueOf(requestedGroup), String.valueOf(assignedKey),
	 ...
	 * 
	 * @param keyStroke
	 *            key stroke to convert to human readable text
	...
	private static class ShortcutCollection extends CopyOnWriteArrayList<Shortcut> {
		...
		public boolean add(Shortcut shortcut) {
			...
			if (stream().map(Shortcut::getShortText).anyMatch(shortcut.getShortText()::equals)) {
				logger.warning(shortcut.getShortText() + " already added");
			}
	...
	private static Optional<Shortcut> findShortcutByKeyOrShortText(int requestedKey, int modifier, String shortText) {
		...
		return shortcuts.stream().filter(sameKey.or(sameShortText)).sorted(Comparator.comparingInt(sc -> sameShortText.test(sc) ? 0 : 1))
				.findAny();
	}
	...
	private static int[] keys = { KeyEvent.VK_F1, KeyEvent.VK_F2, KeyEvent.VK_F3, KeyEvent.VK_F4, KeyEvent.VK_F5, KeyEvent.VK_F6,
	...
	private static void doInit() {
		if (initdone)
			return;
		initdone = true;
		int commandDownMask = Flexo.platformHook.getMenuShortcutKeyMaskEx();
		groups.put(NONE, -1);
		...
		Flexo.platformHook.initSystemShortcuts();
		// (2) User defined shortcuts
		// TODO: handle this one day
		/*Main.pref.getAllPrefixCollectionKeys("shortcut.entry.").stream().map(Shortcut::new)
	...
	public static boolean savePrefs() {
		return shortcuts.stream().map(Shortcut::save).reduce(Boolean.FALSE, Boolean::logicalOr); // has changed
	}
	 ...
	 * 
	 * @param shortText
	 *            an ID. re-use a {@code "system:*"} ID if possible, else use something unique.
	 * @param longText
	 *            this will be displayed in the shortcut preferences dialog. Better use something the user will recognize...
	 * @param key
	 *            the key. Use a {@link KeyEvent KeyEvent.VK_*} constant here.
	 * @param modifier
	 *            the modifier. Use a {@link KeyEvent KeyEvent.*_MASK} constant here.
	...
	public static Shortcut registerSystemShortcut(String shortText, String longText, int key, int modifier) {
		...
		if (existing.isPresent() && shortText.equals(existing.get().getShortText())) {
			...
			System.err.println("CONFLICT WITH SYSTEM KEY " + shortText + ": " + existing.get());
			return null;
		}
	 ...
	 *
	 * @param shortText
	 *            an ID. re-use a {@code "system:*"} ID if possible, else use something unique. {@code "menu:*"} is reserved for menu
	 *            mnemonics, {@code "core:*"} is reserved for actions that are part of Openflexo's core. Use something like
	 *            {@code <pluginname>+":"+<actionname>}.
	 * @param longText
	 *            this will be displayed in the shortcut preferences dialog. Better use something the user will recognize...
	 * @param requestedKey
	 *            the key you'd prefer. Use a {@link KeyEvent KeyEvent.VK_*} constant here.
	 * @param requestedGroup
	 *            the group this shortcut fits best. This will determine the modifiers your shortcut will get assigned. Use the constants
	 *            defined above.
	 * @return the shortcut
	 */
	...
	private static Shortcut registerShortcut(String shortText, String longText, int requestedKey, int requestedGroup, Integer modifier) {
		...
		if (existing.isPresent() && shortText.equals(existing.get().getShortText())) {
			...
			if (Flexo.platform == Platform.OSX) {
				...
				if (!findShortcut(requestedKey, newmodifier).isPresent()) {
					logger.info("Reassigning OSX shortcut '" + shortText + "' from Meta to Ctrl because of conflict with " + conflict);
			...
			for (int m : mods) {
				for (int k : keys) {
					...
					if (!findShortcut(k, newmodifier).isPresent()) {
						logger.info("Reassigning shortcut '" + shortText + "' from " + modifier + " to " + newmodifier
	...
	private static int findNewOsxModifier(int requestedGroup) {
		switch (requestedGroup) {
			...
			case ALT_CTRL:
				return KeyEvent.ALT_DOWN_MASK | KeyEvent.CTRL_DOWN_MASK;
			case CTRL_SHIFT:
				return KeyEvent.CTRL_DOWN_MASK | KeyEvent.SHIFT_DOWN_MASK;
			case ALT_CTRL_SHIFT:
				return KeyEvent.ALT_DOWN_MASK | KeyEvent.CTRL_DOWN_MASK | KeyEvent.SHIFT_DOWN_MASK;
			default:
				return 0;
		}
	}

	private static Shortcut reassignShortcut(String shortText, String longText, int requestedKey, Shortcut conflict, int m, int k,
			int newmodifier) {
		Shortcut newsc = new Shortcut(shortText, longText, requestedKey, m, k, newmodifier, false, false);
		logger.info("Silent shortcut conflict: " + shortText + " moved by " + conflict.getShortText() + " to " + newsc.getKeyText() + ".");
		newsc.saveDefault();
		shortcuts.add(newsc);
		return newsc;
	}
	...
	/**
	 * Replies the platform specific key stroke for the 'Copy' command, i.e. 'Ctrl-C' on windows or 'Meta-C' on a Mac. null, if the platform
	 * specific copy command isn't known.
	 *
	...
	/**
	 * Replies the platform specific key stroke for the 'Paste' command, i.e. 'Ctrl-V' on windows or 'Meta-V' on a Mac. null, if the
	 * platform specific paste command isn't known.
	 *
	...
	/**
	 * Replies the platform specific key stroke for the 'Cut' command, i.e. 'Ctrl-X' on windows or 'Meta-X' on a Mac. null, if the platform
	 * specific 'Cut' command isn't known.
	 *
