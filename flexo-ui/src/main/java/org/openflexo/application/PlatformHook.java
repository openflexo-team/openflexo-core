...
public interface PlatformHook {
	...
	/**
	 * The preStartupHook will be called extremly early. It is guaranteed to be called before the GUI setup has started.
	 *
	 ...
	 * associated to an OS request. The non-GUI, platform-specific tasks, are usually performed by the {@code PlatformHook}.
	 * 
	 * @since 12695
	 */
	public interface NativeOsCallback {
		/**
		 * macOS: Called when Openflexo is asked to open a list of files.
		 * 
		 * @param files
		...
		/**
		 * macOS: Invoked when Openflexo is asked to quit.
		 * 
		 * @return {@code true} if Openflexo has been closed, {@code false} if the user has cancelled the operation.
		 */
		boolean handleQuitRequest();

		/**
		 * macOS: Called when Openflexo is asked to show it's about dialog.
		 */
		void handleAbout();

		/**
		 * macOS: Called when Openflexo is asked to show it's preferences UI.
		 */
