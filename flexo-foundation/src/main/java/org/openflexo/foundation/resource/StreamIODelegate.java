...
public interface StreamIODelegate<I> extends FlexoIODelegate<I> {
	...
	public abstract class StreamIODelegateImpl<I> implements StreamIODelegate<I> {
			...
			if ((_diskLastModifiedDate == null || _diskLastModifiedDate.getTime() == 0 || !exists()) && !_isSaving) {
				...
				if (_diskLastModifiedDate.getTime() > _lastWrittenOnDisk.getTime() + ACCEPTABLE_FS_DELAY) {
					if (_lastWrittenOnDisk.getTime() != 0) {
						// Here we have written on disk, and somehow the disk
						// last modified date is still bigger than the
						// acceptable delay
					...
					// the top 'if'), we consider that it is some kind of bug in
					// the
					// FS
					// and we update accordingly so that the resource checking
					// thread won't think that the resource was updated by
					// another
					// application
					_lastWrittenOnDisk = _diskLastModifiedDate;
				}
		...
		public ClassLoader retrieveClassLoader() {
			// Default stream IO does not implement ClassLoader management
			return null;
		}
