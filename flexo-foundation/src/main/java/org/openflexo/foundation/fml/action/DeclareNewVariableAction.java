...
public class DeclareNewVariableAction extends FlexoAction<DeclareNewVariableAction, AssignableAction<?>, FMLObject> {
	...
	protected void doAction(Object context) throws FlexoException {
		...
		getFocusedObject().setO
		declarationAction = factory.newDeclarationAction(getNewVariableName(), getFocusedObject());
		getFocusedObject().replaceWith(declarationAction);*/

		/*FMLControlGraphOwner owner = getFocusedObject().getOwner();
		String ownerContext = getFocusedObject().getOwnerContext();
		Sequence parentFlattenedSequence = getFocusedObject().getParentFlattenedSequence();
		
		owner.setControlGraph(null, ownerContext);
		
		declarationAction = factory.newDeclarationAction(getNewVariableName(), getFocusedObject());
		// getFocusedObject().replaceWith(declarationAction, getFocusedObject().getOwner(), getFocusedObject().getOwnerContext());
		
		// We connect control graph
		getFocusedObject().setOwnerContext(ownerContext);
		...
		if (parentFlattenedSequence != null) {
			parentFlattenedSequence.controlGraphChanged(getFocusedObject());
		}*/
