 ...
 * 
 */
public class BehaviorPasteHandler extends FlexoPasteHandler<FlexoBehaviour> {

	private static final Logger logger = Logger.getLogger(BehaviorPasteHandler.class.getPackage().getName());
	...
	@Override
	public boolean isPastable(FlexoClipboard clipboard, PastingContext<FlexoBehaviour> pastingContext) {
	...
	@Override
	public Object paste(FlexoClipboard clipboard, PastingContext<FlexoBehaviour> pastingContext) {

		FMLControlGraph cgBeeingPasted = (FMLControlGraph) clipboard.getLeaderClipboard().getSingleContents();
		FlexoBehaviour cgWhereToPaste = pastingContext.getPastingPointHolder();
		cgWhereToPaste.getControlGraph().sequentiallyAppend(cgBeeingPasted);
